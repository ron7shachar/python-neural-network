...
class NN_cricher():
    ...
    def performing(self,problem):
        ...
        self.evaluation = net.SGD(
            training_data[:min(len(training_data),self.properties["training_data_size"])],
            self.properties["epochs"],
            10,
            self.properties["learning_rate"],
            test_data=test_data,
            print_=False)
        error = sum(v[True] for v in self.evaluation.values())
    ...
    def update_fitness(self):
        ...
        #self.evaluation
        error = sum(v[True] for v in self.evaluation.values())/self.test_size
